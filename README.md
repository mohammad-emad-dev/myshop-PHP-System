...
- `includes/dashboard.php` owns the bounded dashboard KPI read `dashboard_get_stats()`, sales/purchase chart read `dashboard_get_chart_data()`, and inventory valuation read `dashboard_get_inventory_valuation()`. `public/index.php` calls all three directly with explicit database and staff-scope dependencies where applicable; `includes/functions.php` retains `get_dashboard_stats()`, `get_chart_data()`, and `get_inventory_valuation()` as delegation-only compatibility wrappers.
