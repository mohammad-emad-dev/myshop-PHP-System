...
- `includes/dashboard.php` owns the bounded dashboard KPI read `dashboard_get_stats()`, sales/purchase chart read `dashboard_get_chart_data()`, inventory valuation read `dashboard_get_inventory_valuation()`, and top-selling product read `dashboard_get_top_selling_products()`. `public/index.php` calls all four directly with explicit database and staff-scope dependencies where applicable; `includes/functions.php` retains the corresponding legacy names as delegation-only compatibility wrappers.
