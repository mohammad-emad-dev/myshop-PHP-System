...
- `includes/dashboard.php` owns the bounded dashboard KPI read `dashboard_get_stats()`. `public/index.php` calls it directly with explicit staff scope; `includes/functions.php` retains `get_dashboard_stats()` as a delegation-only compatibility wrapper.
