...
- `includes/categories.php` owns category creation and update write services, including duplicate-name checks and General-category protection. `public/categories.php` calls `categories_create()` and `categories_update()` directly; `includes/functions.php` retains only their legacy wrappers while `delete_category()` remains facade-owned for a later batch.
...
- Remaining database/business workflows, including authentication rate limiting, staff administration, category deletion, and customer/supplier writes, remain in the facade or page controllers until a future refactor establishes and tests their dependency boundaries.
