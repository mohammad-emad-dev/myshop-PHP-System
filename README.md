...
- `includes/dashboard.php` owns the bounded dashboard KPI read `dashboard_get_stats()` and sales/purchase chart read `dashboard_get_chart_data()`. `public/index.php` calls both directly with explicit staff scope; `includes/functions.php` retains `get_dashboard_stats()` and `get_chart_data()` as delegation-only compatibility wrappers.
