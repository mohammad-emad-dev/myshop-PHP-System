...
try {
    ...
    $totalAssertions = $unitAssertions + $deploymentAssertions + $httpHarnessAssertions + $integrationAssertions + $backupAssertions + $operationalAssertions;
    $duration = number_format(microtime(true) - $started, 2);
    echo "PASS: {$totalAssertions} assertions (" . ($unitAssertions + $deploymentAssertions + $httpHarnessAssertions) . " unit, " .
