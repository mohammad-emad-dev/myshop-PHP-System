...
Status: Phase 3E dashboard top-selling extraction baseline
...
| `get_dashboard_stats()`, `get_chart_data()`, `get_inventory_valuation()`, `get_top_selling_products()` | Delegation-only compatibility wrappers to `includes/dashboard.php`; uploads and remaining report functions remain in the facade |
...
| Category sales aggregate functions | Remaining dashboard sales aggregates |
...
- `includes/dashboard.php`: explicit dashboard KPI aggregation, bounded
  sales/purchase chart data, Product inventory valuation, and bounded top-selling
  product reads with explicit optional staff scope where applicable, complete
  chart date series, and preserved zero/empty failure contracts. The
  `get_dashboard_stats()`, `get_chart_data()`, `get_inventory_valuation()`, and
  `get_top_selling_products()` names remain delegation-only compatibility
...
| `public/index.php` | Dashboard authorization, direct `dashboard_get_stats()`, `dashboard_get_chart_data()`, `dashboard_get_inventory_valuation()`, and `dashboard_get_top_selling_products()` calls, remaining report data preparation, dashboard view |
...
- `includes/functions.php`: legacy full product reads, remaining inventory,
  ...
  `get_chart_data()`, `get_inventory_valuation()`, and `get_top_selling_products()`
  are compatibility wrappers for `includes/dashboard.php`.
